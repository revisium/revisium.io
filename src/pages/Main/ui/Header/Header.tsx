import { Flex, Link, Text } from '@chakra-ui/react'
import { observer } from 'mobx-react-lite'
import { FC } from 'react'
import { LuExternalLink } from 'react-icons/lu'
import { MainPageModel } from 'src/pages/Main/model/MainPageModel.ts'
import { useColorModeValue } from 'src/shared/ui'

interface HeaderProps {
  model: MainPageModel
}

export const Header: FC<HeaderProps> = observer(({ model }) => {
  const bg = useColorModeValue('#FFFFFF', '#0a0a0a')
  const shadow = useColorModeValue('0 1px 0 0 #e5e5e5', '0 1px 0 0 #262626')
  const logoColor = useColorModeValue('#171717', '#e5e5e5')
  const linkColor = useColorModeValue('#525252', '#d4d4d4')
  const linkHover = useColorModeValue('#171717', '#f5f5f5')

  return (
    <Flex
      as="header"
      position="sticky"
      top={0}
      zIndex={10}
      w="100%"
      bg={bg}
      boxShadow={shadow}
      justifyContent="space-between"
      alignItems="center"
      px="1rem"
      h="3.75rem"
    >
      <Flex align="center" gap="1.5rem">
        <Link href="/" display="flex" alignItems="center" gap="8px" _hover={{ textDecoration: 'none' }}>
          <LogoIcon />
          <Text fontSize="16px" fontWeight={600} color={logoColor}>
            Revisium
          </Text>
        </Link>
        <Link
          href={model.header.docsLink}
          fontSize="1rem"
          color={linkColor}
          _hover={{ color: linkHover, textDecoration: 'none' }}
        >
          {model.header.docsLabel}
        </Link>
      </Flex>
      <Flex align="center" gap="1rem">
        <Link
          href={model.header.githubLink}
          target="_blank"
          rel="noopener noreferrer"
          display="flex"
          alignItems="center"
          gap="4px"
          fontSize="1rem"
          color={linkColor}
          _hover={{ color: linkHover, textDecoration: 'none' }}
        >
          {model.header.githubLabel}
          <LuExternalLink size={16} />
        </Link>
      </Flex>
    </Flex>
  )
})

const LogoIcon = () => (
  <svg width="32" height="32" viewBox="0 0 420 420" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
    <path
      d="M25.7474 25.7474C0 51.4947 0 92.9345 0 175.814V244.186C0 327.066 0 368.505 25.7474 394.253C51.4947 420 92.9345 420 175.814 420H244.186C327.066 420 368.505 420 394.253 394.253C420 368.505 420 327.066 420 244.186V175.814C420 92.9345 420 51.4947 394.253 25.7474C368.505 0 327.066 0 244.186 0H175.814C92.9345 0 51.4947 0 25.7474 25.7474Z"
      fill="white"
    />
    <path
      d="M151.395 105C141.955 105 134.302 112.653 134.302 122.093C134.302 131.533 141.955 139.186 151.395 139.186C160.836 139.186 168.488 131.533 168.488 122.093C168.488 112.653 160.836 105 151.395 105Z"
      fill="black"
    />
    <path
      d="M268.605 183.14C259.164 183.14 251.512 190.792 251.512 200.233C251.512 209.673 259.164 217.326 268.605 217.326C278.045 217.326 285.698 209.673 285.698 200.233C285.698 190.792 278.045 183.14 268.605 183.14Z"
      fill="black"
    />
    <path
      d="M268.605 280.814C259.164 280.814 251.512 288.467 251.512 297.907C251.512 307.347 259.164 315 268.605 315C278.045 315 285.698 307.347 285.698 297.907C285.698 288.467 278.045 280.814 268.605 280.814Z"
      fill="black"
    />
    <path
      fillRule="evenodd"
      clipRule="evenodd"
      d="M34.186 151.395C34.186 96.1423 34.186 68.5158 51.3509 51.3509C68.5158 34.186 96.1424 34.186 151.395 34.186H268.605C323.858 34.186 351.484 34.186 368.649 51.3509C385.814 68.5158 385.814 96.1423 385.814 151.395V156.344C385.814 168.503 386.407 177.41 389.887 184.763C393.367 192.115 406.66 195.791 420 195.791V224.209C406.66 224.209 393.367 227.885 389.887 235.237C386.407 242.59 385.814 251.497 385.814 263.656V268.605C385.814 323.858 385.814 351.484 368.649 368.649C351.484 385.814 323.858 385.814 268.605 385.814H151.395C96.1424 385.814 68.5158 385.814 51.3509 368.649C34.186 351.484 34.186 323.858 34.186 268.605V263.656C34.186 251.497 33.5929 242.59 30.113 235.237C26.6331 227.885 13.3396 224.209 0 224.209V195.726C13.3396 195.726 26.6331 192.05 30.113 184.698C33.5929 177.346 34.186 168.438 34.186 156.279V151.395ZM109.884 122.093C109.884 99.1668 128.469 80.5814 151.395 80.5814C174.322 80.5814 192.907 99.1668 192.907 122.093C192.907 140.773 180.571 156.565 163.605 161.779V188.023H228.918C234.132 171.057 249.925 158.721 268.605 158.721C291.531 158.721 310.116 177.306 310.116 200.233C310.116 223.159 291.531 241.744 268.605 241.744C249.925 241.744 234.132 229.408 228.918 212.442H163.605V285.698H228.918C234.132 268.731 249.925 256.395 268.605 256.395C291.531 256.395 310.116 274.981 310.116 297.907C310.116 320.833 291.531 339.419 268.605 339.419C249.925 339.419 234.132 327.083 228.918 310.116H151.395C144.652 310.116 139.186 304.65 139.186 297.907V161.779C122.22 156.565 109.884 140.773 109.884 122.093Z"
      fill="black"
    />
  </svg>
)
